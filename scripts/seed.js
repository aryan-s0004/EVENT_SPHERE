...
(async () => {
  try {
    ...
    for (const u of USERS) {
      ...
      const label = created ? '✅  Created' : 'ℹ️   Already exists (updated)';
    ...
    for (const e of EVENTS_TEMPLATE) {
      ...
      const label = created ? '✅  Created' : 'ℹ️   Already exists (skipped)';
    ...
    for (const b of bookingSeeds) {
      ...
      if (exists) {
        console.log(`    ℹ️   Booking already exists: user ${b.user} → event ${b.event}`);
        continue;
      }
