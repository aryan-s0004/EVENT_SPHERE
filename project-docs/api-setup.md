...
```
  ...
  Note: The database name `EVENTSPHERE` is case-sensitive. MongoDB Atlas will error if you connect with a different casing.
...
3. Go back to Security → search App Passwords or go directly to https://myaccount.google.com/apppasswords
...
```
  ...
  Note: On Vercel, use port `465` (SSL) — port `587` (STARTTLS) is often blocked by AWS outbound rules.
