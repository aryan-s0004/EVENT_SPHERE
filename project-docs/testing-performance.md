...
```
baseUrl    = https://event-sphere-navy.vercel.app
...
# Against production
k6 run -e BASE_URL=https://event-sphere-navy.vercel.app tests/k6-load-test.js
```
...
| No DB indexes on `otp`, `email` | Slower OTP lookups at scale | `email` is already indexed (unique); `otp` not needed — looked up by `userId` |
...
4. **MongoDB Atlas Search** for full-text event search — replaces regex `$options: 'i'`
